<template>
  <div class="container">
    <div class="max-w-2xl mx-auto text-gray-300">
      ...
      <p>Legacy vETH2 exits are handled through protocol liquidity or the legacy withdrawal queue when governance funds and finalizes requests. The queue uses a configured redemption rate and FIFO finalization, so exit timing depends on available liquidity, governance settings, and queue state.</p>
      <p>SharedStake has also supported vETH2 liquidity through <a href="https://saddle.finance/">Saddle Finance</a> as an alternative exit path.</p>
      ...
      <h2>What is vETH2?</h2>
      <p>vETH2 is SharedStake’s legacy token for ETH staked through the original staking contracts. Legacy redemptions are now expected to follow the configured old-vETH2 withdrawal queue rate and FIFO lifecycle. You can check out blocks generated by SharedStake's validators on <a href="https://beaconcha.in/blocks?q=sharedstake.org">the beacon chain</a>.</p>
