...
// File contracts/governance/TokenMigrator.sol
// ref code from gnt
...
contract TokenMigrator is Ownable {
    ...
    constructor(IERC20 _oldToken, IBlocklist bl) public {
    ...
    function migrateFrom(address _from, uint256 _value) external {
        ...
        address to;
        if (blocklist != address(0) && blocklist.inBlockList(msg.sender)) {
            target.burn(_value);
    ...
    function setBlocklist(IBlocklist _blocklist) external onlyOwner {
        emit BlocklistChanged(blocklist, _blocklist);
