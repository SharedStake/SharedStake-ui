...
usage() {
  cat <<USAGE
Usage: $0 [--network <name>] [--skip-install] [--no-sync] [--sync-target <path>]
...
if [[ ! -d "$SHAREDDEPOSIT_DIR/node_modules" ]]; then
  if [[ "$SKIP_INSTALL" -eq 1 ]]; then
    die "SharedDeposit dependencies are missing. Remove --skip-install or run npm ci in SharedDeposit."
  fi
  if [[ -f "$SHAREDDEPOSIT_DIR/yarn.lock" ]] && command -v yarn >/dev/null 2>&1; then
    log "Installing SharedDeposit dependencies with yarn --frozen-lockfile"
    (
      cd "$SHAREDDEPOSIT_DIR"
      yarn install --frozen-lockfile
    )
  else
    require_cmd npm
    log "Installing SharedDeposit dependencies (prefer npm ci, fallback npm install)"
    (
      cd "$SHAREDDEPOSIT_DIR"
...
log "Compiling SharedDeposit contracts"
...
if ! (
  ...
  die "SharedDeposit compile failed. See $COMPILE_LOG_PATH"
fi
log "Compile finished. Log: $COMPILE_LOG_PATH"

log "Deploying SharedDeposit contracts to network: $NETWORK"
...
if ! run_hardhat_deploy; then
  ...
  if grep -Eq "for a different chainId|cannot get the transaction for .+previous deployment" "$DEPLOY_LOG_PATH"; then
    DEPLOYMENTS_DIR="$SHAREDDEPOSIT_DIR/deployments/$NETWORK"
    if [[ -d "$DEPLOYMENTS_DIR" ]]; then
      BACKUP_DIR="$SHAREDDEPOSIT_DIR/deployments/${NETWORK}.stale-deployments.$(date +%Y%m%d%H%M%S)"
      ...
      if ! run_hardhat_deploy; then
        print_log_tail "$DEPLOY_LOG_PATH" 120
        die "SharedDeposit deploy failed after stale-deployments recovery. See $DEPLOY_LOG_PATH"
      fi
    else
      die "Deploy failed with stale deployment metadata but no deployments directory found. See $DEPLOY_LOG_PATH"
    fi
  else
    die "SharedDeposit deploy failed. See $DEPLOY_LOG_PATH"
  fi
fi
...
ARTIFACT_PATH="$GENERATED_DIR/shareddeposit-$NETWORK-addresses.json"
